...
def bninception(num_classes=1000, pretrained='imagenet', model_dir='/home/lulu/project/tsn-pytorch/pretrain_model'):
    ...
    if pretrained is not None:
        ...
        model.load_state_dict(model_zoo.load_url(settings['url'], model_dir=model_dir))
...
class AffluxBN(nn.Module):
    ...
    def __init__(self, num_classes=1000, pretrained='imagenet'):
        ...
        self.inception_5b_relu_pool_proj = nn.ReLU(inplace)
        self.last_linear = nn.Linear(1024, num_classes)
        self.build_substream(pretrained, num_classes)

    def build_substream(self, pretrained='imagenet', num_classes=1000,
                        model_dir='/home/lulu/project/tsn-pytorch/pretrain_model'):
        if pretrained is not None:
            settings = pretrained_settings['bninception'][pretrained]
            assert num_classes == settings['num_classes'], \
                "num_classes should be {}, but is {}".format(settings['num_classes'], num_classes)
            self.load_state_dict(model_zoo.load_url(settings['url'], model_dir=model_dir))
        ...
        self.fuseion_model=nn.Conv2d(128, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

        # copy the parameters to sub stream
        if pretrained is not None:
            mp = list(self.parameters())
            mcp = list(self.substream.parameters())
            n = len(mcp)
            for i in range(0, n):
                mp[i].data[:] = mcp[i].data[:]

    def features(self, image, flow):
        conv1_7x7_s2_out = self.conv1_7x7_s2(image)
        conv1_7x7_s2_bn_out = self.conv1_7x7_s2_bn(conv1_7x7_s2_out)
        conv1_relu_7x7_out = self.conv1_relu_7x7(conv1_7x7_s2_bn_out)
        pool1_3x3_s2_out = self.pool1_3x3_s2(conv1_relu_7x7_out)
        stream_output = self.substream(flow)
        concatenate_feature=torch.cat([pool1_3x3_s2_out,stream_output],1)
        pool1_3x3_s2_out=self.fuseion_model(concatenate_feature)
        conv2_3x3_reduce_out = self.conv2_3x3_reduce(pool1_3x3_s2_out)
        conv2_3x3_reduce_bn_out = self.conv2_3x3_reduce_bn(conv2_3x3_reduce_out)
        conv2_relu_3x3_reduce_out = self.conv2_relu_3x3_reduce(conv2_3x3_reduce_bn_out)
        conv2_3x3_out = self.conv2_3x3(conv2_relu_3x3_reduce_out)
    ...
    def forward(self, input, flow):
        x = self.features(input, flow)
        x = self.logits(x)
        return x


class Substream(nn.Module):
    ...
    def forward(self, x):
        ...
        conv1_7x7_s2_bn_out = self.conv1_7x7_s2_bn(conv1_7x7_s2_out)
        conv1_relu_7x7_out = self.conv1_relu_7x7(conv1_7x7_s2_bn_out)
        y = self.pool1_3x3_s2(conv1_relu_7x7_out)
        return y
